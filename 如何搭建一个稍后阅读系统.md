...
## （3）功能3：将网页中的图片自动下载到本地，防止网页内容失效

**目的**：我希望实现的功能是网页中的图片自动下载到本地，防止网页内容失效

**工具**：使用插件`Local Images Plus`

## （4）功能4：在浏览器中就可以查看网页是否被收藏、状态是已读/未读/正在读
...
使用工具：
1. `JumpVideo`[^2]（这个不是插件，而是软件。使用的不是开源的那个）
2. obsidian插件：`Media Extended`（如果想要它支持bilibili视频和时间戳笔记，就不给你安装最新的v4版本，而是v3版本，比如v3.2.6）

## （6）功能6：AI系统

工具：`copilot插件`（能用的免费配置方法[^1]）

## （7）功能7：筛选出正在看/已看完/未看的笔记
...
## （8）功能8：如何将视频的字幕扒下来，放在视频所在的markdown文件中（这样即使在视频内容失效后，至少保留了字幕内容）

1. 如果你保存的是哔哩哔哩的视频，因为它一般会有自动生成的字幕，所以你可以安装一个浏览器插件：**哔哔君 - bilibili哔哩哔哩字幕列表**[^4]。然后你就可以通过复制视频旁边的字幕到视频所在的markdown文件中。youtube可能也有类似的工具，比如：YouTube Subtitle Downloader。
2. 如果你保存的视频不是哔哩哔哩的视频或者没有字幕，那么你可以将其下载下来，然后给到诸如**飞书妙计**等软件生成字幕，然后复制到markdown所在文件中。
...
[^4]: [哔哔君 - bilibili哔哩哔哩字幕列表 - Chrome 应用商店 (google.com)](https://chromewebstore.google.com/detail/%E5%93%94%E5%93%94%E5%90%9B-bilibili%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%E5%AD%97%E5%B9%95%E5%88%97%E8%A1%A8/bciglihaegkdhoogebcdblfhppoilclp)